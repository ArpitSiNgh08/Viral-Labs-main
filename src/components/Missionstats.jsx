...
const STAT_STEPS = [
  { value: 100, label: "Viral Impressions" },
  { value: 200, label: "Content Pieces Delivered" },
  { value: 900, label: "Views Generated" },
];
...
export default forwardRef(function MissionStats(props, ref) {
  ...
  // ── Stat cycling: odometer initial state ──
  useGSAP(() => {
    const cols = digitCols.current.filter(Boolean);
    const labelEl = statLabelRef.current;
    const dots = dotsRef.current.filter(Boolean);
    if (cols.length < 3 || !labelEl || dots.length < 3) return;

    const orange = "#FF6F21";
    const white = "#FFFFFF";

    const initialDigits = String(STAT_STEPS[0].value).padStart(3, "0").split("").map(Number);
    initialDigits.forEach((d, i) => {
      gsap.set(cols[i], { yPercent: -(d * 10) });
    });
  ...
  function _animateToStep(targetStep) {
    ...
    // Kill without snapping — new tween picks up from current position
    ...
    // Update step immediately so next call knows where we're headed
    currentStepRef.current = targetStep;

    const orange = "#FF6F21";
    const white = "#FFFFFF";
    const nextStep = STAT_STEPS[targetStep];
    const nextDigits = String(nextStep.value).padStart(3, "0").split("").map(Number);
    ...
    // Dots crossfade
    tl.to(dots[fromStep], {
      backgroundColor: white, duration: 0.4, ease: "power2.inOut",
    }, 0);
    tl.to(dots[targetStep], {
      backgroundColor: orange, duration: 0.4, ease: "power2.inOut",
    }, 0.25);
  }
  ...
  function startAutoCycle() {
    ...
    for (let j = 0; j < STAT_STEPS.length; j++) {
      ...
      nextDigits.forEach((d, colIdx) => {
        tl.to(cols[colIdx], {
          yPercent: -(d * 10), duration: 0.9, ease: "power3.inOut",
        }, pos + colIdx * 0.1);
      });

      tl.to(labelEl, {
        yPercent: -120, opacity: 0, duration: 0.35, ease: "power3.in",
      }, pos);
    ...
    <div ref={wrapperRef} className="relative w-full min-h-screen overflow-visible">
     <div className="relative mx-auto w-[calc(100%-clamp(2.5rem,10vw,13.125rem))] max-w-[106.875rem] min-h-screen flex flex-col">
      ...
      >
        ...
        >
          <div className="flex flex-col gap-0 w-full lg:max-w-[22.5rem] 4xl:max-w-[28rem] 5xl:max-w-[34rem] max-sm:w-full">
            ...
            <h2 className="impact-heading font-host font-medium text-[clamp(2rem,4.5vw,3.375rem)] max-sm:text-4xl max-mob:text-[1.875rem] max-mob:leading-[1.05] max-mob:tracking-[-1.5px] 4xl:text-[4rem] 5xl:text-[5rem] text-[#f1f1f1] tracking-[-2.16px] leading-[1.0]">
              Proven Impact
            </h2>
          </div>
      ...
      <div className="relative z-40 px-[clamp(0.75rem,3vw,3rem)] max-sm:px-2 max-mob:px-3 4xl:px-16 5xl:px-20 pb-10 my-auto">
        ...
        >
          <p className="impact-quote font-host text-[clamp(1.5rem,3.3vw,2.5rem)] max-sm:text-2xl max-mob:text-[1.375rem] max-mob:leading-[1.15] max-mob:tracking-[-0.5px] 4xl:text-[3rem] 5xl:text-[3.5rem] text-[#f1f1f1] text-center tracking-[-1.6px] leading-[0.95] w-full max-w-[26.25rem] 4xl:max-w-[32rem] 5xl:max-w-[38rem] max-sm:w-full">
            Performance isn't accidental. It's deployed.
          </p>
        </div>
          ...
          <div className="relative w-full max-w-[1064px] 4xl:max-w-[1300px] 5xl:max-w-[1500px] aspect-[1064/346] overflow-hidden max-sm:w-[115%] max-mob:w-[110%]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pt-[clamp(1rem,2.5vw,3.5rem)] max-sm:pt-5">
              <div className="flex flex-col items-center gap-0 text-center">
                ...
                <div
                  className="stat-number flex items-center font-bricolage font-normal text-[clamp(2.5rem,6vw,5.5rem)] max-sm:text-5xl max-mob:text-4xl 4xl:text-[6.5rem] 5xl:text-[8rem] text-white tracking-[-0.06em] max-sm:tracking-[-2px] max-mob:tracking-[-2px] leading-none whitespace-nowrap opacity-0 translate-y-8"
                  style={{ fontVariationSettings: "'opsz' 14, 'wdth' 100" }}
                >
                  {[0, 1, 2].map((colIndex) => (
                    <div
                      ...
                      style={{ height: "1em" }}
                    >
                      <div ref={(el) => (digitCols.current[colIndex] = el)}>
